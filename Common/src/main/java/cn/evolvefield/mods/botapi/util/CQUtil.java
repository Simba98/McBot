...
public class CQUtil {
    ...
    public static String replace(String msg, boolean image_on) {
        if (Services.PLATFORM.isModLoaded("chatimage") && image_on) {
            var matcher = RegexUtil.regexMatcher(CQ_CODE_REGEX, msg);//匹配cq码格式
                    ...
                    Arrays.stream(matcher.group(2).split(","))//具体数据分割
                            ...
                            .forEach(args -> {
                                if (args.substring(0, args.indexOf("=")).equals("url")) {//url
                                    var v = unescape(args.substring(args.indexOf("=") + 1));//去除空格
