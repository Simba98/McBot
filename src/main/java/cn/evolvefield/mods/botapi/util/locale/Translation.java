...
public abstract class Translation {
    ...
    private static Translation loadDefault(String langId) {
        ...
        return new Translation() {
            ...
            public IReorderingProcessor getVisualOrder(ITextProperties formattedText) {
                return (formattedCharSink) -> {
                    return formattedText.visit((style, string) -> {
                        return TextProcessing.iterateFormatted(string, style, formattedCharSink) ? Optional.empty() : ITextProperties.STOP_ITERATION;
