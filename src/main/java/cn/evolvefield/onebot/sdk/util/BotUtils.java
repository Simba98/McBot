...
import cn.evolvefield.onebot.sdk.entity.ArrayMsg;
import cn.evolvefield.onebot.sdk.entity.MsgChainBean;
...
public class BotUtils {
    ...
    private static Logger log = LogManager.getLogger("BotUtils");
    ...
    public static List<MsgChainBean> stringToMsgChain(String msg) {
        ...
        try {
            Arrays.stream(msg.split(CQ_CODE_SPLIT)).filter(s -> !s.isEmpty()).forEach(s -> {
                var matcher = RegexUtils.regexMatcher(CQ_CODE_REGEX, s);
                ...
                if (matcher == null) {
                    ...
                    Arrays.stream(matcher.group(2).split(",")).filter(args -> !args.isEmpty()).forEach(args -> {
                        ...
                        var v = BotUtils.unescape(args.substring(args.indexOf("=") + 1));
